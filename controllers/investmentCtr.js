...
const reInvestment = asyncHandler(async (req, res) => {
  const amount = parseFloat(req.body.amount);
  const invest = await Investment.findById(req.body.id);
  if (parseFloat(invest.amount) >= amount) throw new Error(`Invalid amount`);
  await Wallet.findOneAndUpdate({ user: req.user._id }, { $inc: { binance: parseFloat(invest.amount) - amount } });
